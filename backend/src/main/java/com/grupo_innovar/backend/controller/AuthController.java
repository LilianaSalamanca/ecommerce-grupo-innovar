...
public class AuthController {
    ...
    public ResponseEntity<?> register(@RequestBody RegisterRequest r) {
        ...
        u.setActivo(true);
        u.setInvitado(false);
        u.setTipoUsuario(Usuario.TipoUsuario.PUBLICO);
        ...
        return ResponseEntity.ok(
                new LoginResponse(token, u.getEmail(), u.getNombre())
        );
    }
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest r) {
        ...
        try {
            ...
            Usuario usuario = repo.findByEmail(r.email()).orElseThrow();
            ...
            return ResponseEntity.ok(
                    new LoginResponse(token, usuario.getEmail(), usuario.getNombre())
            );

        } catch (DisabledException e) {

            return ResponseEntity.status(403)
                    .body(Map.of("message", "Cuenta desactivada. Contacte al administrador."));
    ...
    public ResponseEntity<LoginResponse> loginWithGoogle(
        ...
        if (usuario == null) {
            ...
            usuario.setTipoUsuario(Usuario.TipoUsuario.PUBLICO);
        ...
        return ResponseEntity.ok(
                new LoginResponse(token, email, usuario.getNombre())
        );
    }
}